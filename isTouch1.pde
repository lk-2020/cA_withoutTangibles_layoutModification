...
void ifTouchEventIs1()
{
  ...
  if ((rightHandJoint0<bufferArea)&&(rightHandJoint1<whiteHandLength+bufferArea)&&(rightHandJoint1>whiteHandLength-bufferArea)) // right joint id touchId[0]
  {
    ...
    angle11[rec][iterations[rec]] = atan2(dy1, dx1);
    strokeWeight(25);
    stroke(skeletonColor);
    ...
    //angle[it] = angle1[it];
    angle1_ = angle11[rec][iterations[rec]];
  ...
  else if ((leftHandJoint0<bufferArea)&&(leftHandJoint1<whiteHandLength+50)&&(leftHandJoint1>whiteHandLength-50)) // right joint id touchId[0]
  {
    ...
    angle11[rec][iterations[rec]] = atan2(dy2, dx2);
    //angle[it] = angle2[it];
    //segment(whiteLeftHandJointX, whiteLeftHandJointY, angle2, whiteHandLength);
    //  the ink cretaes 2 points at a predefined distance on the tangible and hence this distance will always remain const.
    touchEvent = true;
    touchEvent_1[rec][iterations[rec]] = true;
    yellow_x1[rec][iterations[rec]] = (animatingFaceX-whiteSkeletonXincr) - (whiteLeftHandJointX - whiteLeftHandEndX);
    yellow_y1[rec][iterations[rec]] = whiteLeftHandEndY-15;
    jYellow_x1[rec][iterations[rec]] = animatingFaceX-whiteSkeletonXincr;
    yellow_xL_ =(float)yellow_x1[rec][iterations[rec]];
    yellow_yL_ = (float)yellow_y1[rec][iterations[rec]];
    jYellow_xL_ = (int)jYellow_x1[rec][iterations[rec]];
    angle2_ = angle11[rec][iterations[rec]];
